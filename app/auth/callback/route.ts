...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!/^[A-Za-z0-9_-]{43}$/.test(flow.verifier) || typeof flow.issued !== 'number' || Date.now()-flow.issued > 600000 || flow.issued > Date.now() || !adminEmail()) return finish('/dang-nhap?error=google_expired');
    ...
    if (!user.email_confirmed_at || !googleIdentity || email !== adminEmail()) return finish('/dang-nhap?error=google_forbidden');
    const response = finish(safeReturnTo(typeof flow.destination === 'string' ? flow.destination : null));
    response.cookies.set(sessionCookie, await newSession(email), {...cookieOptions(), maxAge:sessionLifetime});
    return response;
